...
from data_processing_utils import window_truncate, random_mask, add_artificial_mask, \
    saving_into_h5, set_seed

if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='Generate UCI Energy dataset')
    ...
    logger = setup_logger(os.path.join(dataset_saving_dir + "/dataset_generating.log"),
                          'Generate UCI electricity dataset', mode='w')
    logger.info(args)
    ...
    df = df[(df.minute==0)|(df.minute==30)]

    unique_months = df['datetime'].dt.to_period('M').unique()
    
    selected_as_test = unique_months[3:4]  # select first 10 months as test set
    logger.info(f'months selected as test set are {selected_as_test}')
    selected_as_val = unique_months[4:]  # select the 11st - the 20th months as val set
    logger.info(f'months selected as val set are {selected_as_val}')
    selected_as_train = unique_months[:3]  # use left months as train set
